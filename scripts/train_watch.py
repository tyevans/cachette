...
# The name the shared controller baseline carries on the screen. The line it
# prints names no strategy, because one process measures the number for all
# of them.
...
def read(text: str) -> Reading:
    ...
    **A line that carries the marker of work in flight is never a result.**
    ...
    for line in text.splitlines():
        ...
        if shard_count:
            named(shard_count.group("name")).sharding = int(
                shard_count.group("shards")
            )
            continue
        if in_flight(line):
