...
| 0055 | A god raises the ground its people hold, and sees what stands there. **The project owner asked for it directly.** It sits directly under 0008 because it is the next statement of the same need: 0008 lets a unit leave a mark, and this asks that the mark fit the ground, have a level a watcher reads, and follow a plan the faction makes. It sits above 0003 because two of its statements are what the map must show, and 0003 is the record that the map answers to. |
...
| 0047 | A game states its own economy. **The engine holds every economic knob and the control plane reaches none of them**, so a downstream game states its production, its upkeep, its cadence, its stores and its recovery only by forking the engine. It sits directly above 0020 because it is the economic part of 0030 and it ships without any new engine mechanism: every value it names already governs the engine and already has a unit and a scale. |
| 0045 | A god knows its congregation by name. **The engine holds a whole character tier that no caller can reach**, and the need it answers belongs to the game the project owner named. It sits beside 0031 and 0032 because it is the same shape: a fixed-size read over what the engine already holds, with no new simulation behind it. It sits under 0034 because a deal between two players moves goods and this moves nothing, so it is the smaller change of the two. |
| 0046 | A god raises somebody up. It sits directly under 0045 because a caller that cannot read the people has nothing to write to, and because every write it names is refused for an identity that a read must supply first. |
| 0040 | A god inflicts weather on a place. **The project owner asked for it directly**, and the engine now answers it: a god names a set of places, the engine refuses a place its faction does not hold, and the ground there is wet on the next frame. It sits beside 0031 and 0034 because it comes from the same game and it takes the same gate. It sits above 0030 because it is one bounded piece of that need that already ships, and BLK-130 holds the only thing still open about it, which is what the power should cost. |
...
| 0048 | A developer watches factions play a game to an end. **The project owner asked for a game layer inside the engine, and this is the need it serves.** It sits above 0049, 0050, 0051 and 0052 because each of those is one mechanism a faction plays with, and none of them has a reason to fire until something wants to win. It sits under 0036 because it drives the verbs that 0034, 0035 and 0040 already answer, so those must hold before a faction can use them. |
| 0049 | A god declares war and makes peace. It sits directly under 0048 because a faction that wants to win must choose an enemy, and because it gates the contest that fires everywhere today. It sits above 0050 and 0051 because a standing between two factions is what refuses a deal at war, so trade needs it first. |
| 0050 | A god advertises what it will trade. It sits under 0049 because a faction that acts on its own trades only where a posted want meets its surplus, and a deal is refused at war. It sits above 0051 because ground is one more thing a post can name, and the post must exist before it names anything. |
| 0051 | A god trades land. It sits under 0050 because it widens the deal that 0034 answers, and above 0052 because BLK-036 governed both. That row is now resolved: an upgrade changes hands with the ground, so a land deal carries what stands on the tile and needs no refusal. |
...
| 0057 | Four ways to win exist and, measured, one of them decides nearly every game. It sits directly under 0053 because 0053 builds the report and this record states what the report must show, and above 0056 because a learner rated in an arena with one live way to win is rated on one axis. It is not new mechanism: three of the four readers exist, and what is open is whether each one can be reached and steered toward. BLK-050 governs every share and every target, and BLK-150 governs the source of one path. |
| 0056 | A learner plays one faction against the controllers. It sits directly under 0057 because a policy needs the game that 0048 to 0052 build, it needs the fairness that 0053 judges, and it needs the several live ways to win that 0057 asks for. It waits for the territory rule and the upgrade table, because each of those changes what a learner would observe and what it would do. The reader of a faction's standing on each way to win already exists. |
