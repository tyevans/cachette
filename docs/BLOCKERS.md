...
**Owner:** the project owner. **Blocks:** the job that publishes the
documentation site.

**A record binds how the reference is built and says nothing about where it
goes.** The published Python reference is generated by an import of the compiled
extension module, and the job that builds it fails when the import
fails.[^BLK35A] The documentation plan holds the structure and the order of the
work.[^BLK35B]

**Two facts are missing, and neither is in the tree.** The first is the address
the site answers on. The second is the source setting of the hosting, which a
static site host reads from a repository setting rather than from a file, and
which the research report names as a requirement of the workflow it
documents.[^BLK35C] No file in this repository states either, and no contributor
can read them from the tree.

**This is information and not a judgement.** A guess produces a job that builds
correctly and publishes nowhere, or one that publishes to an address nobody
gave. The item that adds the job therefore states the address as a parameter and
cites this row.
