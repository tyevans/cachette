...
| 0472 | Pass 1 of the living world game layer, and the tracer bullet for the nine rows under it. **Nothing in the demonstration world wants anything, and no run ends.** After it a faction controller runs inside the step, the game ends on territory, and the census says what every subsystem produced. It sits directly under 0471 because the programme is the current work of the project owner, and under 0471 because a hole in the hash is repaired before a stage is added to the step. It touches `fn step`, so one worker holds it at a time. |
| 0473 | Pass 2. Every later pass reads a unit type column that the row does not hold. It sits beside 0472 because it touches no stage and may run in parallel with it. |
| 0474 | Pass 3. Two adjacent factions always fight, so no faction can be at peace. It sits under 0473 because a hostile unit and a permitted trade are both defined by the band it adds, and it touches `fn step`, so it waits for 0472 to merge. |
...
| 0477 | Pass 6. A faction cannot say what it wants, and a contract moves only a resource. It sits under 0476 because it touches no stage and may run beside 0478 and 0479 once 0476 merges. BLK-036 governs the land kind. |
| 0478 | Pass 7. A faction at war does nothing about it. It sits beside 0477 because it reads the band of 0474 and the soldier row of 0473 and adds no stage. |
| 0479 | Pass 8. Only one win path exists after 0472, and it fires only at the tick limit. It sits beside 0477 and 0478 because it adds no stage. BLK-150 governs the renown reader. |
| 0480 | Pass 9. A watcher cannot see a relation, a board, a storm or a score. It sits here because it runs beside 0474 to 0479, one panel as each subsystem lands, and because 0316 should land before five panels are written against the old shape. |
| 0481 | Pass 10. Nothing says whether the game is fair, and every row of the balance register is unset until this pass measures it. It sits last of the programme because it needs every win path. |
