...
| 0277 | Hold a thread back when the work will not pay for it. The influence solve is 5.9 times slower at twelve threads than at one on the demonstration world, and it does not cross over at any extent measured up to a million tiles. **It sits first because it is the only row here that is worth more than it costs at every extent below the target one, and because it is the reason the demonstration runs at the rate it does**: the frame is between two and three times faster with fewer threads and no engine change. It is a decision before it is a change, because any work-per-thread rule holds a constant the current code was written to avoid. FND-300 holds the measurement. |
...
| 0300 | Two of the panel line kinds draw past the right edge of the panel and over the map. **It is a class rather than an instance**: a row cuts its value to the space left and a note and a founding row cut nothing, so every note that fits today fits by accident. It sits below 0278 because nothing is visibly wrong after the one overflowing note was shortened, and above the later rows because the panel is the surface a watcher opens when a card is not enough, and a panel that writes over the map is not readable. |
