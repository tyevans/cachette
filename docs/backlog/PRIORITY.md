...
| 0307 | Give the panel one standard that a new panel registers with. It sits directly under 0300 because it is how 0300 is fixed: the cut moves into one writer that every line kind uses, so the class cannot recur, and a second panel becomes one new file rather than an edit to a shared method. |
| 0309 | Show the characters, the statistics, the events and a tile in panels. **A faction that dies is invisible until it is gone**, because every unit count the panel states is a count of the window. It sits under 0307 because each panel is a file that the standard of 0307 registers. |
| 0308 | Separate the engine tick from the wall clock in the demonstration. A watcher cannot stop the world, and the logs the panel reports last one tick, so a promotion is readable for one thirtieth of a second. It sits under the two panel rows because it changes what a watcher can read rather than what the panel holds. |
