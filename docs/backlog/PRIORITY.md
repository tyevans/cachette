...
| 0268 | Hold the cell index on the unit. It stores the cell of a unit beside the tile of the unit, for about 4 MB. It took the place 0267 held, because the measurement that refused 0267 priced this one in the same run: the two steps it removes are one hardware division, and they are the largest single part of the chain both items were drawn from. It still carries a real risk, which is a second declaration site for a value the tile already fixes, and that is the defect shape this project records most often. The item refuses to be done without the check that fails when the two disagree, and that check is written first. |
