...
| 0512 | Add the temperature term to the production pipeline. **The pipeline composes four terms and the project owner asked for a fifth**, so a site's output follows temperature in the same way it follows moisture. It sits directly under 0513 because 0513 builds the pipeline this term is added to, and it cannot start before that merges. It is also held by a measurement: the temperature field is not built and its range is not taken, so a weight written today would be an invented value. |
| 0476 | Pass 5. Weather moves water and changes nothing else. **It sat under 0475, and 0475 is complete**, so the wear step it names now exists and this row is what is left: the harm that weather does to a store, to production, to a unit and to movement. The upgrade half is done and the other four are not. It touches `fn step`, so one worker holds it at a time. BLK-130 governs every rate it reads. |
...
| No. | Why it sits here |
|---|---|
| 0532 | Train an attention policy over the entity tokens, and rate it against the policy the project trains today. **The project owner asked for it directly**, and the arithmetic says it is affordable: the readout and the scalar layer hold three quarters of the trainable count, so an attention head over the token sets costs a few percent of population and the smallest design costs none at all. It sits at the top of `Next` rather than in `Now` because it cannot start today: most power ratios of a rival token read zero, and the rival set is the set the design is for. ADR-0201 governs it and forbids the ring-cell variant. BLK-007 governs every cost figure it states. |
| 0539 | Restart a reclaimed training run on a new instance. **A reclaimed run now keeps its weights and cannot use them**, because the launcher sends no resume point to a machine it has just made. One run lost the best held-out figure this project has measured before the fetch existed, and FND-746 holds the case. It sits directly under 0525 because both are about what a rented training run costs, and below it because 0525 moves the throughput of every run while this one shortens the recovery after a reclaim. It is in `proposed/`, and refining it is the work: **it must say what a resume point does when the new instance builds a different engine**, because the trainer refuses a checkpoint written under another feature transform. |
...
| 0149 | The cost section of PRD-0018 states the mechanism of ADR-0080. A review holds the record at `Shaped` until it states cost alone.[^5] |
...
| 0166 | The footnote baseline holds every document the new check would fail. It can only shrink, and it does not shrink by itself.[^4] |
...
[^4]: Findings register, FND-130. `docs/FINDINGS.md`
