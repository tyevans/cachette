...
| 0345 | Resolve a meeting between two factions. Two factions stand on one place and nothing happens. **The granularity is settled: a fight resolves at the tile, because the casualty band is 1 tile wide there and up to 30 tiles wide at the level 1 cell.** FND-390 holds the measurement and DEC-144 is closed on it. It still waits on 0343 for the unit type, and on DEC-145, which is the project owner's to make. DEC-170 is new and it asks what a contest reads, because a rule that needs two factions on one tile never fires against an army packed to the capacity of its ground. |
