...
| 0310 | Write the Rust doc comments for the Python reader. It sits second because the reference publishes empty prose until the doc comments exist, and because the generated page is the thing that makes each gap visible. ADR-0107 says in its own consequences that no check can see this gap. |
