...
| 0266 | Order the unit arena by cell. The measurement half is done and the cheaper half of the fix shipped: the movement pass now walks the order the bridge already holds, so it reads the tile side in ascending tile order. What is left is the unit columns, which are indexed by the slot and stay scattered. It waits on a target-platform run that states the residual and the reorder in one process, because the reorder is priced at 64 milliseconds for one million units and the residual is not priced at all. The figure this row once gave, 2.11, measured the density of the population and not the arena order. |
