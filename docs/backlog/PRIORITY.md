...
| No. | Why it sits here |
|---|---|
| 0292 | Make the change merge take a thread count, or make it move less. It is 43.6 milliseconds and it runs on the calling thread. **It is the largest serial pass in the engine**, and a serial pass bounds every thread count above it. It sits first because the frame no longer has one dominant stage: the largest is the holding spread at 28.3 percent of a 249.0 millisecond frame, and this is 17.5 percent. Four stages take no thread count and this is three quarters of them. |
