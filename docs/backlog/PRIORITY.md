...
| 0269 | Map the large arrays with huge pages. It is the cheapest experiment the project has: no algorithm, no layout and no arithmetic changes, so it cannot touch determinism. It sits above the layout items because it costs almost nothing to try and because a translation cost would appear exactly where the stage split leaves 62 percent of the unit cost unattributed. |
| 0270 | Score the option set with integer vector instructions. The pass has a fixed option count, one shared weight profile and dense columns, and the invariant against floating point is what makes a vector result bit-identical to the scalar one. **The project pinned a stable toolchain when this item was written, so the portable vector library was not available**; ADR-0097 moved the project to a dated nightly for this item, and the portable library is now the fallback where intrinsics used to be. It sits below the layout items on purpose: a vector unit fed one scattered value at a time is a scalar unit with extra steps. |
| 0267 | Hold the exit direction on the tile. A unit reads its direction through four scattered lookups, and the answer could sit at the unit's own tile index for about 67 MB against 31 GB of unused memory. It sits above 0268 because it removes the whole chain rather than two steps of it, and below 0238 because the field it reads must exist first. Its risk is that the derive gains a pass over every tile, which the item says to measure. |
...
| No. | Why it sits here |
|---|---|
| 0272 | The move to a dated nightly opened a door the compiler used to hold shut: the reassociating float methods now compile, and only the script's name check stands in it. The lint can name them and does not. FND-263 holds the measurement and DEC-107 must close first. It sits at the top of `Next` because it is the one thing the toolchain move made worse before anything makes it better, and because the fix is three lines and a check. |
| 0274 | Nothing fails when the toolchain file names a floating channel, and one deleted word is the whole distance between the rule and its violation. ADR-0097 D2 is prose today. It sits beside 0272 because both guard something the move to nightly put at risk, and above 0273 because the failure it prevents is invisible until a golden state hash differs between two checkouts of one commit. |
| 0275 | The Miri gate has only ever passed, so its reach is unmeasured, and the project's own rule calls a test with no proven failure mode decoration. The gate exists to catch a defect every other gate waves through, and nobody has shown that its fixture reaches the case. It sits below 0274 because a gate that may not fire is a smaller loss than a rule nothing enforces, and above 0273 because it can be finished without a decision closing first. |
| 0273 | Three files state which compiler this project needs and they now disagree in kind, with nothing that fails when they part company. ADR-0097 names the consequence and leaves it open. It sits last of the four because the manifest's claim is still true today: nothing uses an unstable feature yet, so the disagreement is a trap rather than a lie. Refine it before the first pass that uses the portable vector library, not after. |
