...
| 0308 | The documentation plan. It is the umbrella item for the seven rows below it, and it closes when they do. It sits directly under 0307 because it states the order those seven run in, and because it decided that no documentation build starts before 0307: a reference built over two declaration sites publishes whichever site the tool reached. BLK-035 governs it, because nobody in the tree can say where the site publishes. |
<<<<<<< HEAD
| 0310 | Write the Rust doc comments for the Python reader. It sits first of the six because the reference publishes what the doc comments say, and because the generated page is the thing that makes each gap visible. **The gap is not an absence.** Every public member of the compiled module already carries a docstring, and FND-325 holds the count, so what is left is the audience: a comment written for a contributor to the core under-serves the Python developer PRD-0021 names. ADR-0107 says in its own consequences that no check can see that. |
=======
| 0309 | Publish the Python reference generated from the compiled module. It is first of the seven because it is the only quadrant that is derived rather than written, so it cannot decay quietly, and because the other three quadrants cite it. ADR-0107 D1 and D4 bind it, and D4 is why it is a compile job: a build that falls back to the type stub succeeds and publishes a page with signatures and no prose. |
