...
| 0474 | Pass 3. Two adjacent factions always fight, so no faction can be at peace. It sits under 0472 because a hostile unit and a permitted trade are both defined by the band it adds, and it touches `fn step`, so it waits for 0472 to merge. Pass 2 is complete, so the unit type columns it reads exist. |
...
| 0477 | Pass 6, the engine half. A faction cannot say what it wants, and a contract moves only a resource. It sits under 0476 because it touches no stage and may run beside 0478 and 0479 once 0476 merges. BLK-036 governs the land kind. |
| 0482 | Pass 6, the controller half. A faction that holds a board never writes to it. It sits under 0477 because it writes the board 0477 adds, and it waits for the controller stage of 0472 and the band of 0474. |
