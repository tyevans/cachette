...
| No. | Why it sits here |
|---|---|
| 0340 | Answer whether one faction stands in the territory of another, in one call. **It is the central mechanic of a game the project owner named**, and the engine already holds every part of the answer. A tile carries a holder, a unit carries a faction and a tile, and the control plane can only read one tile at a time. The answer for the whole world is one word for each faction, so it cannot be made slow by a large world, and deriving it rides on a pass that already visits every unit. FND-362 holds the reasoning and DEC-141 holds the options. It sits at the top because nothing else on this list is a stated need that the engine can already answer. |
| 0341 | Bind the build order and the upgrade removal to the control plane. **The core holds four public build methods and no binding calls one.** FND-360 holds the search. It sits second because it is the cheapest ratio of value to work on this list, and because it closes a capability that ships inert today, which is a shape the rules already name. |
...
| No. | Why it sits here |
|---|---|
| 0342 | Let the control plane name the seed set of a strategy field. **The reach field is built and the control plane cannot name a destination**, because the record fixes the seeds at the live sites of a faction. FND-363 holds it. One change gives two of the six things the downstream game asks for: move units somewhere, and gather units in a place. It sits at the top of `Next` because it amends a draft rather than an accepted record, so DEC-142 must close before anybody writes code. |
| 0344 | Measure whether a fight makes a front line. A level 1 cell covers a block of tiles, and a fight resolved there kills units across the whole block. Two factions have never been run into contact, so there is no evidence either way, and BLK-052 holds the gap. It sits above the combat item because it decides between two designs, and building either one first risks throwing it away. |
| 0343 | Give a soldier a type that indexes a table. **A soldier carries no type**, so the project owner's acceptance test for combat is not expressible. It sits below the measurement because the measurement needs no type, and above the contest because the contest cannot start without one. Read it against 0266, because adding a column while the unit layout is moving costs the work twice. |
| 0346 | Read the units of a faction as a set. **Nothing lists the units of a faction**, so a caller keeps every identity a spawn returned for the life of the run. It sits beside 0161, which states the same gap from the write side, and refining one should decide whether they are one item. |
| 0347 | Read the tile holder as a column. The engine holds the holders as one dense column and the control plane reads one address at a time. It sits below 0346 because the selector may absorb it, and above the later rows because a map of who holds what is what a watcher of a territorial game most wants. |
...
| No. | Why it sits here |
|---|---|
| 0345 | Resolve a meeting between two factions. Two factions stand on one place and nothing happens. It sits here rather than higher because it waits on 0343 for the unit type and on 0344 for the granularity, and because two choices must close first: where the fight resolves, and whether an attacker below the threshold contributes exactly zero. DEC-144 and DEC-145 hold them, and the second is the project owner's to make. |
