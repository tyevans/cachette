...
value for each one, and pushes into a vector sized to the tile count. At the
...
second one, which named a faction and was not a holder.[^3] This item covers
...
- **The reader of the stub value is not surveyed.** The drawing pass reads
  `world.tile_values()`, and the state hash reads the stub column, so every
...
[^3]: Backlog item 0084. `docs/backlog/complete/0084-give-a-tile-one-faction-column.md`
