...
cause no source.[^F484H] The record says so, and the balance row says so.[^F484D]
...
tile.[^F484H] A cell is 32 tiles a side, so a cell can hold ground of two
...
### FND-567 — A test held its own copy of the renown target and went stale
...
### FND-573 — Two fixtures pinned a balance constant, and each measured a different one
...
### FND-637 — The objective took seven values, and a whole training run had nothing to rank
...
### FND-647 — A verb the control plane runs between two steps left the world unreadable, and the refusal named none of its cause
...
world, and the opening banner of the run.[^F647A]
...
calls.[^F647C] The reader carries a refusal that names its cause, so a stale
structure states both revisions and a structure that was never built says
so.[^F647D] The two failure families of the trainer are not shown to be this
...
[^F647A]: Findings register, FND-644. `docs/FINDINGS.md`
[^F647B]: The test that drives each verb and reads afterwards. `crates/cachette-core/tests/a_step_leaves_the_world_readable.rs`
[^F647C]: ADR-0018, the unit-to-tile bridge is derived, and it rebuilds at the barrier, decisions D3 and D4. `docs/adrs/accepted/adr-0018-the-unit-to-tile-bridge-is-derived-and-rebuilds-at-the-barrier.md`
[^F647D]: The refusal of a faction reader. `crates/cachette-core/src/faction_view.rs`
[^F647E]: Findings register, FND-646. `docs/FINDINGS.md`
