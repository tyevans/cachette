...
### FND-488 — A faction that founds with two people fills no ranked position, and the world still ends
...
that number moves shares and nothing else.[^F488A]
...
that the derivation lapsed.[^F488A]
...
[^F488A]: Balance register, the founding group, the base reach and the campaign cohort size. `docs/reference/balance.md`
