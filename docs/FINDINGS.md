...
### FND-293 — The spread read the ground of every candidate tile, and the ground can only refuse
...
the seed rather than stored, so the read is a draw and not a load.**[^F293A]
...
looks like an array index.[^F293A] Nothing at the call site says that it costs a
