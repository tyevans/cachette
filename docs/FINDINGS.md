...
### FND-579 — A city could never change hands, so a war could take the ground and the people and end nothing
...
of a faction returned to it as soon as the lease of an invader decayed.[^F579B]
...
### FND-580 — Two facts about one conquest lived in two columns, and nothing failed when they disagreed
...
reach of every city.[^F580A] A release that cleared the holder and left the
...
### FND-581 — Two guards this work added cannot be reached, and the register says so rather than the tests
