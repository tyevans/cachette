...
### DEC-115 — Which builder publishes the Python reference, and what does the decision record bind?

**Open. Engineering owns it. Nothing is blocked on it, because no documentation
build exists yet.**

The project owner named one candidate, Zensical, and asked for it to be tested
rather than confirmed. Research report 19 holds the evidence, every version
number and every measured figure.[^DEC115A]

**The load-bearing question came out in the candidate's favour.** Zensical does
generate API documentation from docstrings, through a rewrite of the
mkdocstrings plugin driving the original handler package. A probe build against
this repository's own compiled module produced the Rust doc comment prose, the
navigation, the search index and the footnotes.[^DEC115A]

**The candidate is alpha, and the fallback is one command away.** The published
package declares an alpha development status, and the project roadmap says the
same. Zensical reads `mkdocs.yml` natively, so the same configuration file
builds with Material for MkDocs. The risk sits in the tool and not in the
input.[^DEC115A]

**Option A. Bind the builder.** Write a record that names Zensical. This pins a
release line of an alpha tool inside a record body, which the record scope rule
forbids.[^DEC115B]

**Option B. Bind the configuration format and the docstring source.** Write a
record that states two constraints: the published reference is generated by
importing the compiled extension module, and the docstring lives in the Rust
doc comment and not in the type stub. Name the configuration format, not the
builder. The builder then stays replaceable.

**Option C. Defer.** Write no record and add no documentation build. The
project has no release, so nothing depends on a published site today.

**Recommendation: Option B.** It records the constraint that a future
contributor could reasonably choose otherwise on, and it leaves the tool free.
The second constraint is the expensive one: a build that reads the type stub
instead of the module produces a reference with signatures and no prose, and
the report measured that fall.[^DEC115A]

**What closes this.** A decision record that states the two constraints, and a
documentation job that builds the extension before it builds the site.
...
[^DEC115A]: Research report 19, the documentation toolchain. `docs/research/reports/19-documentation-toolchain.md`
[^DEC115B]: Decision Record Scope, section 4.2. `.claude/rules/adr-scope.md`
