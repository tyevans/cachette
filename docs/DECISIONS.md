...
### DEC-117 — Does the publishing job keep a switch of its own, now that the address is known?
...
**Closed. Option B. ADR-0107 holds it.**[^DEC112C]

The record binds the provenance of the documentation prose and not the choice
of builder. It states that the published Python reference is generated by an
import of the compiled extension module, that the prose of a member the module
provides lives in the Rust doc comment, that the type stub carries prose only
for a declaration the module does not provide, and that the documentation job
builds the extension before it builds the site and fails when the import fails.

**Option A failed the second condition of the scope test, and that is the whole
reason.** A record buys the right to refuse a change, and it is only worth
writing when a different choice costs more than a change made later. The site
configuration is a portable format that more than one builder reads, and the
report measured the swap as near free.[^DEC112A] So the builder is named in the
consequences of the record as the tool the project uses now, and it is not the
claim. The record scope rule also forbids the release pin that naming an alpha
tool invites.[^DEC112B]

**The constraint the record exists for is the second one.** A build that reads
the type stub instead of the module publishes a reference with signatures and no
prose, and it reports no error while it does so. A contributor could reasonably
turn module inspection off, because the tool's own handler documentation
recommends it when a stub exists.[^DEC112A] Nothing in the artefact says why that
is wrong here.

**What this did not decide.** Whether the site configuration stays in the
portable format is a separate choice, and it now has its own row.[^DEC112D]
### DEC-116 — Where does the panel layout standard live, and what is one panel?
...
[^DEC112A]: Research report 19, the documentation toolchain. `docs/research/reports/19-documentation-toolchain.md`
[^DEC112B]: Decision Record Scope, section 4.2. `.claude/rules/adr-scope.md`
[^DEC112C]: ADR-0107, the Python reference is generated from the compiled module. `docs/adrs/draft/adr-0107-the-python-reference-is-generated-from-the-compiled-module.md`
...
[^DEC117A]: The documentation site job. `.github/workflows/docs.yml`
