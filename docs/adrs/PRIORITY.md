...
| 0096 | Cost follows the lattice, not the population, and a unit is a reader. **The owner set this above every other item**, and it writes down a principle the orientation has always stated and no record has ever held. **The engine does not satisfy it**, so a reviewer must read it as an argument against the code rather than a description of it. Its D3 corrects the obvious explanation for why the unit passes do not scale: they obey ADR-0009 completely and partition on the axis that satisfies it and destroys locality. Its D4 makes one sentence of accepted ADR-0064 D1 false, and DEC-096 holds how the project repairs a stale consequence. **The author of this record must not review it.** |
...
[^PRI97]: Backlog item 0274, fail a check when the toolchain names a floating channel. `docs/backlog/proposed/0274-fail-a-check-when-the-toolchain-names-a-floating-channel.md`
[^PRI97B]: Backlog item 0275, prove that the Miri gate can fail. `docs/backlog/proposed/0275-prove-that-the-miri-gate-can-fail.md`
