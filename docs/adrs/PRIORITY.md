...
| No. | What it claims |
|---|---|
| 0120 | A trade negotiation is engine state, and the words are not. **The code implements every decision and the tests start at the Python boundary.** A reviewer should test D3 hardest, because it puts the presence rule of the downstream game inside the engine on the strength of one sentence the project owner wrote about messages, and nothing says a trade is a message. D4 is the one to distrust on behaviour: it bounds a pair to one live negotiation, and a game that wants two deals at once meets that bound at once. D5 is the claim that will look wrong to a reader who expects privacy, and the record answers it by pointing at the control plane rather than by refusing the read. **The author of this record must not review it.** |
| 0121 | A terminal refusal closes an ordered pair until a named tick. **The code implements it and a test attempts to reopen a closed pair and reads the refusal.** A reviewer should test D2 hardest, because the closure is directional and the direction is easy to write the wrong way round: closing the speaker's own row rather than the other party's leaves the other party free to ask again, which is the opposite of the promise, and only one test separates the two. D1 is the claim to distrust on design: it hands a player a duration and the engine invents nothing, so a player that names the largest duration has closed the pair for the life of the run. **The author of this record must not review it.** |
| 0122 | A contract moves a quantity only when a unit carries it onto the ground of the other party. **The code implements it and a test asserts that a store rose because a unit stood on it.** A reviewer should test D1 hardest, because a transfer between two stores is shorter, exact and order-free, and the argument against it is about the game rather than about the engine. D4 is the one to distrust: it keeps what already arrived and closes the direction for the term of the contract, and neither half is forced by anything the engine holds. D2 states that a delivery never passes the debt, and that is the clause a reviewer can check against the code in one read. **The author of this record must not review it.** |
