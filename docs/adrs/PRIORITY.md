...
| 0078 | Descent is a bounded record, and a relation is a bounded recursion. The code that implements it exists and its tests pass. Walking two lines to a common ancestor is the obvious implementation and the accepted product record rejects it, so a contributor could reasonably choose otherwise. It also fixes the visit order of a walk, which is a determinism property. |
...
| 0088 | A tile field is a generated base and a stored change. Three tile fields now sit outside the dense column record, and nothing states the rule that picks between the two shapes. The code that implements it exists, and a visit census proves that building a world stores nothing for the field. A review found it sound in substance and returned one bullet of D1, which says a build visits no tile of the field and contradicts a finding the register already holds.[^18] It moves when that bullet is corrected. |
| 0090 | A tile upgrade is stored sparsely. A dense array over the tiles is what a future contributor would reach for, and the code shows the sparse map without showing why the dense one was refused. The implementation exists and every decision was checked against it. A review found it sound in substance and returned one sentence of D3, which claims that every caller asking how many units a tile holds composes the upgrade, where three read the ground alone.[^18] It moves when that sentence is corrected. |
