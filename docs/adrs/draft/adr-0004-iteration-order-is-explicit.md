...
- **A reduction that is order-free** may combine in any order. Integer
  addition and bitwise OR are order-free, because they are associative and
  commutative on exact values. A parallel sum of integers needs no ordering
  work.
- **A reduction that is not order-free needs a slot.** Minimum, maximum, and
  first-wins all depend on order when values tie. Each such reduction writes
  into a slot indexed by a stable key, and the combine step reads the slots in
  index order.
