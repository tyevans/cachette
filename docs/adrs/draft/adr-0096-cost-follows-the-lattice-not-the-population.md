...
the thread count runs out. The measurements are on the target platform, and the
reference register holds them with their derivation.[^3] **No figure appears in
this record**, because a measurement can change and a record must not hold
material that changes.[^4]
...
record already forbids exactly that.[^5]
...
derived from the data rather than from the schedule.[^5] [^6]

It scales poorly anyway, and the reasons are visible in the source rather than
in a profile. The pass collects every live unit into one list before any thread
starts, and it applies the results by walking that list again afterwards. Both
...
from a per-unit search.[^8] A behavioural strategy arrives as a field over cells
and never as a search from a unit.[^9] Both are D2 applied to one subsystem, and
...
and it is not sufficient.**[^5] A pass can satisfy that rule completely, as the
...
still governs it.[^10] What changes is where the work happens. Two units whose
...
record derived, not a decision it made, and how the project repairs a stale
consequence inside an accepted record with dependents is an open question that
this record does not settle.[^12]
...
slot.[^13] The project chose the generational identity deliberately, and this
...
record says in its own consequences that a reviewer cannot see a violation.[^14]
...
separately.[^15] An item carries it.[^16]
...
has measured under the new shape.[^18] An item holds it.[^17]
...
different reason: nothing gives two units different weights.[^11] A project that
...
decision gets no failure. A reader must treat this as unenforced.[^18]
...
[^5]: ADR-0009, parallel stages write disjoint outputs, decision D1. `docs/adrs/accepted/adr-0009-parallel-stages-write-disjoint-outputs.md`
...
[^7]: ADR-0064, a unit chooses by scoring a small fixed option set, decision D4. `docs/adrs/accepted/adr-0064-a-unit-chooses-by-scoring-a-small-fixed-option-set.md`
...
[^9]: ADR-0095, a behavioural strategy arrives as a field over cells, never as a search from a unit, decision D1. `docs/adrs/draft/adr-0095-a-behavioural-strategy-arrives-as-a-field-over-cells.md`
[^10]: ADR-0064, a unit chooses by scoring a small fixed option set, decision D1. `docs/adrs/accepted/adr-0064-a-unit-chooses-by-scoring-a-small-fixed-option-set.md`
[^11]: Findings register, FND-251. `docs/FINDINGS.md`
[^12]: Decisions register, DEC-096. `docs/DECISIONS.md`
