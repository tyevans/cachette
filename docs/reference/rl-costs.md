...
| Reserved position count | The observation builder | 1, provisional | — | Declared as `LAYOUT_RESERVE`. A revision that claims a position takes it from here and holds the length, so no position moves. Five revisions have claimed positions, and the commit message of each states what it took[^21] |
| Layout revision integer | The policy fit of the control plane | 8 | — | Declared as `OBSERVATION_VERSION`. It rises when a position moves or a published value changes, and each rise retires every stored policy[^21] |
