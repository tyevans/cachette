...
| Terrain height range | 0 to 4000 m | BLK-156 | The terrain declares a height as a unit fraction and this is what the whole fraction is worth. **The owner chose 4000 m for the alpine contrast, then replaced it with 1500 m on 7 September 2026 to stop the lapse rate taking 11 to 18 degrees off every land cell.** That cooling was a defect in the ground term and not a cost of the range: the term measured every cell from sea level, where the balance constants already average over the land of a planet. The term now reads a cell against the mean land height of the world, so the range sets how far cells stand apart and not how cold the world is, and the reason for the smaller figure no longer holds. The team lead restored 4000 m on the owner's behalf against a measurement, and the owner may put 1500 m back in one line. **The larger range restores the alpine contrast the owner wanted first**, and the highest ground is a mountain rather than a high hill. **A world states the range, in the way a world states its latitude span**, so a world that wants different ground changes a value and not the physics |
