...
| Pressure scale, the pressure one step of heat gives | The wind pass | unset, pass 10 | BLK-130 | Provisional default declared as `PRESSURE_DIVISOR_AT_REFERENCE` in the weather module, at the reference lattice pitch. A finer lattice holds a smaller difference between two neighbours, so a solve reads the divisor from the pitch and the constant states only the reference. Written by item 0499. Pass 10 measures it. |
