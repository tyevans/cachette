...
use cachette_core::faction_observation::FieldRow;
...
fn row(world: &World, name: &str) -> FieldRow {
    world
        .observation_schema()
        .row(name)
...
/// The block holds the same shape on every world.
///
/// A field whose length or bound followed the world shape would make one
/// policy unable to read two worlds. Every other block of the array has a
/// length that follows the tile count, the faction count or the cell count,
/// and this one has none.
#[test]
fn the_block_holds_one_shape_on_every_world() {
    let small = World::new(WorldConfig {
        width: 4,
        height: 4,
        seed: LAND_SEED,
        faction_count: 2,
    ...
    .expect("a world of sixteen tiles is a world");
    let large = World::new(WorldConfig {
        width: 128,
        height: 96,
        seed: LAND_SEED + 1,
        faction_count: 9,
    ...
    .expect("a wide world is a world");
    ...
    for name in [
        ...
        let thin = row(&small, name);
        let thick = row(&large, name);
        assert_eq!(
            thin.positions, thick.positions,
            "the field `{name}` must hold one length on every world"
        );
        assert_eq!(
            (thin.low, thin.high),
            (thick.low, thick.high),
            "the field `{name}` must hold one bound on every world"
        );
    }
}
