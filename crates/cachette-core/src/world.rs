...
impl World {
    ...
    fn check_game_end(&mut self) {
        ...
        // **The wealth or wonder path has no reader here.** It ended 32 of 32
        // seeds at the tick limit while domination and territory ended none,
        // and the owner ruled that a store passing a number is not a way to
        // win a war. The reader stays, and the path keeps its number, so a
        // caller that wants it can read it and a stored record still names it.
    ...
    #[allow(dead_code, reason = "the path has no reader in the table today")]
