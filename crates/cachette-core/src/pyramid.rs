    ...
    pub fn derive(&mut self, pyramid: &Pyramid) {
        ...
        for cell in 0..cells.tile_count() {
            ...
            for (option, row) in OPTIONS.iter().enumerate() {
                ...
                //
                // [^13]: ADR-0110, a unit returns by climbing a reach field seeded at every site of its faction, decision D1. `docs/adrs/draft/adr-0110-a-unit-returns-by-climbing-a-reach-field.md`
...
/// that made it.[^1]
///
/// **The guard state is not part of the answer either.** The arguments of the
/// last derivation and the count of the derivations say how the field reached
/// what it holds. Two fields that hold one answer by two routes are equal, and
/// a comparison that read the route would call them different.
///
/// # References
///
/// [^1]: ADR-0022, level 0 is the only truth, and every level above it is derived, decision D2. `docs/adrs/accepted/adr-0022-level-0-is-the-only-truth-and-every-level-above-it-is-derived.md`
