...
pub(crate) mod tests {
    ...
    /// Founds a city for the faction, steps, and returns one tile the
    /// faction holds that admits a unit.
    ///
    ...
    /// wants one thing, a tile whose holder is the named faction, so it
    /// founds a city and lets the holding pass state the rule. A fixture that
    /// walked the rule would be a second declaration of it.[^2]
    ///
    /// # References
    ///
    ...
    pub(crate) fn a_held_tile(world: &mut World, faction: FactionId) -> Axial {
        let seat = (20..40)
            ...
            .find(|at| world.admits_a_unit(*at) && world.settlements().on_tile(*at).is_none())
            .expect("the band holds open ground with no city on it");
        world
            .found_settlement(seat, faction)
